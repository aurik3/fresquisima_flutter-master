...
class FoldingOrder extends StatefulWidget {
  ...
  FoldingOrder({this.duration, @required this.entries, this.isOpen = false, this.onClick});

  @override
  _FoldingOrderState createState() => _FoldingOrderState();
}

class _FoldingOrderState extends State<FoldingOrder> with SingleTickerProviderStateMixin {
  List<FoldEntry> _entries;
  double _ratio = 0.0;
  AnimationController _controller;

  double get openHeight => _entries.fold(0.0, (val, o) => val + o.height) + FoldingOrder.padding * 2;

  double get closedHeight => _entries[0].height + FoldingOrder.padding * 2;
  ...
  @override
  void didUpdateWidget(FoldingOrder oldWidget) {
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: EdgeInsets.all(FoldingOrder.padding),
